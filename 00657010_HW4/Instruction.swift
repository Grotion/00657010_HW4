...
struct appUsage: View
{
    var body: some View
    {
        VStack
        {
            ...
            .foregroundColor(Color(red: 129/255, green: 0, blue: 0))
            .multilineTextAlignment(.center)
            Text("歡迎使用「專業稽查員Pro」！在頁面下方有三個分頁可以做選擇。此頁為「使用指南」，包含使用方法及說明。第二個分頁為「稽查紀錄」，頁面中左上有兩個按鈕，一個為\"Edit\"能將單一紀錄刪除，另一個為「被發現了！」能將所有的稽查紀錄『全部』刪除，且目前暫無復原系統。頁面右上方有兩個按鈕，一個為「一鍵輸入」能輸入Demo資料(固定，非隨機)，第二個為藍色十字記號能新增新的稽查紀錄，在新增頁面能輸入稽查對象的基本資料，若選擇加入最愛則會被優先顯示於記錄頁面。在稽查紀錄頁面點選任一紀錄能進行編輯。第三個分頁為「稽查分析」，共有4個分析選項，點選任一分析選項將會於下方顯示不同的分析圖表。祝您使用愉快！！")
            .lineLimit(Int.max)
            .padding(EdgeInsets(top: 10, leading: 10, bottom: 0, trailing: 15))
        }
        .frame(height:400)
